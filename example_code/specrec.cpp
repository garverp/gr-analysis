...
std::string create_metadata_header( double samp_rate, double freq, double gain) {
	...
	header = pmt::dict_add(header, pmt::mp("type"), pmt::from_long(1));
	header = pmt::dict_add(header, pmt::mp("cplx"), pmt::PMT_F);
	header = pmt::dict_add(header, pmt::mp("rx_time"), pmt::make_tuple(pmt::from_uint64(0), pmt::from_double(0) ));//timestamp);
	header = pmt::dict_add(header, pmt::mp("rx_rate"), pmt::mp(samp_rate));
	header = pmt::dict_add(header, pmt::mp("bytes"), pmt::from_uint64(0xFFFFFFFF));
...
template<typename samp_type> void recv_to_file(
   ...
   header_str = create_metadata_header( usrp->get_rx_rate(), usrp->get_rx_freq(), usrp->get_rx_gain());
   std::cout << std::endl << header_str.size() <<std::endl;
   std:: cout << write(fd,header_str.c_str(),header_str.size());
   }
...
#define recv_to_file_args(format) \
   (usrp, format, wirefmt, file, cbcapacity, total_num_samps, total_time, bw_summary, \
    stats, null, enable_size_map, continue_on_bad_packet,metadata)
   //recv to file
