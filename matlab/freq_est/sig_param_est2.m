function [fhatArr, varfhatArrCRL, phihatArr, varphihatArrCRL, ahatArr, varahatArrCRL,DFT_windowed_x] = sig_param_est(x, variance_of_whiteNoise, samplingRate,windowSize)
    % same as sig_param_est.m but with parabolic interpolation
    blkN = 1;
    ...
    N = (length(x) - mod(length(x),windowSize));  %file length rounded of to highest length divisible by windowSize
        for i = 1:N/windowSize
            ...
            parabolicInterpolation = @(ym1,y0,yp1,x0) -1/2*(yp1 - ym1) / (yp1 - 2*y0 + ym1) + x0;
            result = parabolicInterpolation(DFT_windowed_x(max_arg-1),DFT_windowed_x(max_arg),DFT_windowed_x(max_arg+1),max_arg);
            n_norm = (result - NFFT/2 -1)/NFFT; %(-NFFT/2:NFFT/2-1)./NFFT;
    %       fhat = n_norm(max_arg); % scaled freq
            fhat = (n_norm)*samplingRate; % freq in Hz
